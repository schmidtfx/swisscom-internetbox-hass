...
from sc_inetbox_adapter.errors import SwisscomInetboxException, NoActiveSessionException
...
class InternetBoxClient(InternetboxAdapter):
    def __init__(self, hass: HomeAssistant, host: str, password: str, ssl: bool = True, verify_ssl: bool = True):
        super().__init__(host=host, admin_password=password, ssl=ssl, verify_ssl=verify_ssl)
        self._hass = hass
    ...
    async def async_get_hosts(self) -> list[HostEntry]:
        ...
        devices = []
        for d in data:
            mac = (d.get("PhysAddress") or d.get("MACAddress") or d.get("mac") or "").lower()
            ...
            devices.append(HostEntry(mac=mac, ip=ip, hostname=name, active=active_norm, type=device_type))
    ...
    async def async_get_dsl_info(self) -> dict[str, Any]:
        ...
        def _get():
            headers = self._add_auth_header()
            payload = json.dumps({"parameters":{}})
            response = self._send_request("/sysbus/NeMo/Intf/dsl0:getDSLChannelStats", payload, headers)
            return response.json()
