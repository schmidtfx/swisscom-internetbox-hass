...
def _user_schema_with_defaults(user_input):
    user_schema = {
        vol.Optional(
            CONF_HOST, default=user_input.get(CONF_HOST, DEFAULT_HOST_NAME)
        ): cv.string,
    }
...
def _ordered_shared_schema(schema_input):
    return {
        ...
        vol.Optional(
            CONF_SSL, default=schema_input.get(CONF_SSL, DEFAULT_SSL)
        ): cv.boolean,
        vol.Optional(
            CONF_VERIFY_SSL,
...
class SwisscomFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None):
        ...
        client = InternetBoxClient(
            hass=self.hass, host=host, password=password, ssl=ssl, verify_ssl=verify_ssl
        )
        ...
        if (
            info.get("ModelName") is not None
            and info.get("HardwareVersion") is not None
        ):
